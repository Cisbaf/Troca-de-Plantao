package com.trocaplantao.auth.controller;


import com.trocaplantao.auth.entity.UserLogin;
import com.trocaplantao.auth.entity.UserRequest;
import com.trocaplantao.auth.service.UserService;
import com.trocaplantao.auth.service.utils.JwtRequestFilter;
import com.trocaplantao.auth.service.utils.JwtTokenUtil;
...
public class UserController {
    ...
    @PostMapping("/login")
    public ResponseEntity<Map<String, String>> login(@RequestBody @Valid UserLogin login, HttpServletResponse response) {
        try {
    ...
    public ResponseEntity<Map<String, String>> register(@RequestBody @Valid UserRequest userRequest) {
        if (userService.validateCredentials(userRequest.username())) {
            ...
            String username = userService.registerUser(userRequest.username(), userRequest.password(), userRequest.email(), userRequest.name());
    ...
    @GetMapping("/mail-usernames")
    public ResponseEntity<Map<String, String>> getAllEmailsAndUsernames() {
        Map<String, String> emailsAndUsernames = userService.getAllEmailsAndUsernames();
        return ResponseEntity.ok(emailsAndUsernames);
    }
